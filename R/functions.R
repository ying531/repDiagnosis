...
#' @param treatment_name String for the column name of the treatment indicator (=1 means treatment)
#' @param id_name_1 Optional, string for the column name of the cluster id in the original study (specify if the study is clustered)
#' @param id_name_2 Optional, string for the column name of the cluster id in the original study (specify if the study is clustered)
#' @param covariate_formula Optional, formula for covariates to balance, such as "~ age + gender" which means the covariates to balance are "age" and "gender"
#' @param covariates Optional, vector for column names or column number of covariates to balance. At least one of covariates and covariate_formula must be specified. If both are specified, covariate_formula will be taken
#' @param mediation_formula Optional, formula for mediators to balance, which should also include covariates, such as "~ (age + gender + mediator) * treatment", which means "mediator" is viewed as a mediator to balance
#' @param mediators Optional, vector for column names or column number of mediators to balance. At least one of mediation_formula and mediators must be specified. If both are specified, mediation_formula is taken
#' @param alpha Optional, default at 0.1, coverage level for the confidence intervals is 1-alpha
#' @param verbose Optional, default at TRUE, whether to display summary tables and decomposition plots
#' @param if_selective Optional, default at TRUE, whether or not to run selective inference that adjusts for publication bias
#' @param pub_pvalue_threshold Optional, default at 0.05, the threshold for publication bias, i.e., the selective inference CIs are valid conditonal on that the original study p-value is smaller than pub_pvalue_threshold
